...
chosenbuild<-c("B125","B126","B129","B132","B133","B199","B200","B201","B202")
locations<-"D:/ThesisExperiments/OriginalData/Energy/"
dev.off()
all_arima<-function(filename, location=locations){
  setwd("D:/ThesisExperiments/SelectedBuildings")
  #filename = "B133"
  #location="D:/ThesisExperiments/Original data/Energy/"
  buildata<-read.csv(paste(location, filename, ".csv", sep = ""))
  buildatatsday<-ts(buildata$Ef_kWh,frequency = 24)
  buildatatsweek<-ts(buildata$Ef_kWh,frequency = (24*7))
  buildatatsdaytrain<-ts(buildata$Ef_kWh[1:6768],frequency = 24)
  buildatatsweektrain<-ts(buildata$Ef_kWh[1:6768],frequency = (24*7))
  
  #dev.off()jpeg(filename=paste("Day alldata seasonal decom ",filename,".jpeg",sep=""),height=786,width=1048)
  plot(stl(buildatatsday,"per"),main=paste(filename,"by days, Seasonal Decomposition"))
  #dev.off()jpeg(filename=paste("Week alldata seasonal decom ",filename,".jpeg",sep=""),height=786,width=1048)
  plot(stl(buildatatsweek,"per"),main=paste(filename," by weeks, Seasonal Decomposition"))
  #dev.off()jpeg(filename=paste("Day train seasonal decom  ",filename,".jpeg",sep=""),height=786,width=1048)
  plot(stl(buildatatsdaytrain,"per"),main=paste(filename,"by days, Seasonal Decomposition"))
  #dev.off()jpeg(filename=paste("Week train seasonal decom  ",filename,".jpeg",sep=""),height=786,width=1048)
  plot(stl(buildatatsweektrain,"per"),main=paste(filename," by weeks, Seasonal Decomposition"))
  #dev.off()jpeg(filename=paste("Day alldata seasonal ",filename,".jpeg",sep=""),height=786,width=1048)
  seasonplot(buildatatsday,season.labels=T,main=paste("Seasonal Plot",filename,"by days"))
  #dev.off()jpeg(filename=paste("Week alldata seasonal ",filename,".jpeg",sep=""),height=786,width=1048)
  seasonplot(buildatatsweek,season.labels=T,main=paste("Seasonal Plot",filename,"by weeks"))
  #dev.off()jpeg(filename=paste("Day train seasonal ",filename,".jpeg",sep=""),height=786,width=1048)
  seasonplot(buildatatsdaytrain,season.labels=T,main=paste("Seasonal Plot",filename,"by days"))
  #dev.off()jpeg(filename=paste("Week train seasonal ",filename,".jpeg",sep=""),height=786,width=1048)
  seasonplot(buildatatsweektrain,season.labels=T,main=paste("Seasonal Plot",filename,"by weeks"))
  #dev.off()jpeg(filename=paste("Week alldata especgram ",filename,".jpeg",sep = ""),height=786,width=1048)
  buildataesweek<-spec.pgram(buildatatsweek,log="no")
  #dev.off()jpeg(filename=paste("Day alldata especgram ",filename,".jpeg",sep=""),height=786,width=1048)
  buildataesday<-spec.pgram(buildatatsday,log="no")
  #dev.off()jpeg(filename=paste("Week train especgram ",filename,".jpeg",sep=""),height=786,width=1048)
  buildataesweektrain<-spec.pgram(buildatatsweektrain,log="no")
  #dev.off()jpeg(filename=paste("Day train especgram ",filename,".jpeg",sep=""),height=786,width=1048)
  buildataesdaytrain<-spec.pgram(buildatatsdaytrain,log="no")
  #dev.off()
  
  orderedweekbuildata<-order(buildataesweek$spec,buildataesweek$freq,decreasing=T)#ordena los datos del espectrograma de mayor a menor
  ordereddaybuildata<-order(buildataesday$spec,buildataesday$freq,decreasing=T) #ordena los datos del espectrograma de mayor a menor
  
  maxfdaybuildata<-buildataesday$freq[ordereddaybuildata[1]]
  aaday<-24/maxfdaybuildata
  maxfweekbuildata<-buildataesweek$freq[orderedweekbuildata[1]]
  aaweek<-24*7/maxfweekbuildata
  
  BXCXLAMbuildataday<-BoxCox.lambda(buildatatsday,method="loglik")
  BXCXbuildataday<-BoxCox(buildatatsday,BXCXLAMbuildataday) #hace la transformacion boxcox
  BXCXbuildatadaytrain<-BoxCox(buildatatsdaytrain,BXCXLAMbuildataday)
  
  BXCXLAMbuildataweek<-BoxCox.lambda(buildatatsweek,method="loglik")
  BXCXbuildataweek<-BoxCox(buildatatsweek,BXCXLAMbuildataweek) #hace la transformacion boxcox
  BXCXbuildataweektrain<-BoxCox(buildatatsweektrain,BXCXLAMbuildataweek)
  
  dbuildataweek<-ndiffs(BXCXbuildataweektrain,alpha=0.05,test="kpss",max.d=169)# el valor del componente d  estacionario
  Dbuildataweek<-nsdiffs(BXCXbuildataweektrain,m=frequency(BXCXbuildataweektrain),test="ch",max.D=169)# el valor del componente D  estacional
  
  dbuildataday<-ndiffs(BXCXbuildatadaytrain,alpha=0.05,test="kpss",max.d=25)# el valor del componente d  estacionario
  Dbuildataday<-nsdiffs(BXCXbuildatadaytrain,m=frequency(BXCXbuildatadaytrain),test="ch",max.D=25)# el valor del componente D  estacional
  
  #jpeg(filename=paste("Stationary and non seasonal ",filename," day time series.jpeg",sep=""),height=786,width=1048)
  if(dbuildataday>=1){
    if(Dbuildataday>=1){
    autoplot(diff(diff(BXCXbuildatadaytrain,lag=dbuildataday),lag=(Dbuildataday*24)),
             main=paste(filename," stationary and non seasonal time series by day",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  }
    else{
    autoplot(diff(BXCXbuildatadaytrain,lag=dbuildataday),
             main=paste(filename," stationary and non seasonal time series by day",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  }
    }
  else{
    if(Dbuildataday>=1){
      autoplot(diff(BXCXbuildatadaytrain,lag=(Dbuildataday*24)),
               main=paste(filename," stationary and non seasonal time series by day",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
    }
    else{
      autoplot(BXCXbuildatadaytrain,
               main=paste(filename," stationary and non seasonal time series by day",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
    }
  }
  #autoplot(diff(diff(BXCXbuildatadaytrain, lag = dbuildataday),lag = (frequency(buildatatsdaytrain)*Dbuildataday)), main=paste(filename, " stationary and non seasonal time series by day", sep=" ")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  #autoplot(diff(diff(BXCXbuildatadaytrain,lag=dbuildataday),lag=(Dbuildataday*24)),main=paste(filename," stationary and non seasonal time series by day",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  #dev.off()
  
  #jpeg(filename=paste("Stationary and non seasonal ",filename," week time series.jpeg",sep=""),height=786,width=1048)
  if(dbuildataweek>=1){
    if(Dbuildataweek>=1){
      autoplot(diff(diff(BXCXbuildataweektrain,lag=dbuildataweek),lag=(Dbuildataweek*168)),
               main=paste(filename," stationary and non seasonal time series by week",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
    }
    else{
      autoplot(diff(BXCXbuildataweektrain,lag=dbuildataweek),
               main=paste(filename," stationary and non seasonal time series by week",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
    }
  }
  else{
    if(Dbuildataweek>=1){
      autoplot(diff(BXCXbuildataweektrain,lag=(Dbuildataweek*168)),
               main=paste(filename," stationary and non seasonal time series by week",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
    }
    else{
      autoplot(BXCXbuildataweektrain,
               main=paste(filename," stationary and non seasonal time series by week",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
    }
  }
  #autoplot(diff(diff(BXCXbuildatadaytrain, lag = dbuildataday),lag = (frequency(buildatatsdaytrain)*Dbuildataday)), main=paste(filename, " stationary and non seasonal time series by day", sep=" ")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  #autoplot(diff(diff(BXCXbuildataweektrain,lag=dbuildataweek),lag=(Dbuildataweek*168)),main=paste(filename," stationary and non seasonal time series by week",sep="")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  #dev.off()jpeg(filename=paste("Autocorrelation plot ",filename," day time series.jpeg",sep=""),height=786,width=1048)
  autocorplotday<-ggAcf(residuals(Arima(BXCXbuildatadaytrain,order=c(0,1,0),seasonal=c(0,1,0),
                                     lambda=BXCXLAMbuildataday,include.drift=F,include.constant=T,include.mean=T)),
                     lag.max=(80)) #grafico de autocorrelacion, contar las lineas consecutivas que se salen de la franja azul
  print(autocorplotday)
  #dev.off()jpeg(filename=paste("Parcial Autocorrelation plot ",filename," day time series.jpeg",sep=""),height=786,width=1048)
  
  parautocorplotday<-ggPacf(residuals(Arima(BXCXbuildatadaytrain,order=c(0,0,0),seasonal=c(0,0,0),lambda=BXCXLAMbuildataday))
                         ,lag.max=80)#grafico de autocorrelacion parcial, contar las lineas que se salen de la franja azul
  print(parautocorplotday)
  #dev.off(jpeg(filename=paste("Autocorrelation plot ",filename," week time series.jpeg",sep=""),height=786,width=1048)
  
  autocorplotweek<-ggAcf(residuals(Arima(BXCXbuildataweektrain,order=c(0,1,0),
                                         seasonal=c(0,1,0),lambda=BXCXLAMbuildataweek,
                                         include.drift=F,include.constant=T,include.mean=T)),lag.max=(400)) #grafico de autocorrelacion, contar las lineas consecutivas que se salen de la franja azul
  print(autocorplotweek)
  #dev.off(jpeg(filename=paste("Parcial Autocorrelation plot ",filename," week time series.jpeg",sep=""),height=786,width=1048)
  parautocorplotweek<-ggPacf(residuals(Arima(BXCXbuildataweektrain,order=c(0,0,0),seasonal=c(0,0,0),
                                             lambda=BXCXLAMbuildataweek)),lag.max=400)#grafico de autocorrelacion parcial, contar las lineas que se salen de la franja azul
  print(parautocorplotweek)
  #dev.off()
  
  ordereddata<-list(orderedweekbuildata,ordereddaybuildata,aaday,
                    aaweek,dbuildataday,Dbuildataday,dbuildataweek,Dbuildataweek)
  return(ordereddata)
}



all_arima_fit<-function(filename, location=locations,arimaterms=sarimaterms,
                        hoursforecast=24,daystrain=24*7*40){
  setwd("D:/ThesisExperiments/SelectedBuildings")
  #filename = "B133"
  #location="D:/ThesisExperiments/Original data/Energy/"
  buildata<-read.csv(paste(location, filename, ".csv", sep = ""))
  buildatatsday<-ts(buildata$Ef_kWh,frequency = 24)
  buildatatsweek<-ts(buildata$Ef_kWh,frequency = (24*7))
  buildatatsdaytrain<-ts(buildata$Ef_kWh[1:daystrain],frequency = 24)
  buildatatsweektrain<-ts(buildata$Ef_kWh[1:daystrain],frequency = (24*7))
  ...
  pbuildataweek<-as.numeric(arimaterms[1])
  Pbuildataweek<-as.numeric(arimaterms[2])
  pbuildataday<-as.numeric(arimaterms[5])
  Pbuildataday<-as.numeric(arimaterms[6])
  
  
  qbuildataweek<-as.numeric(arimaterms[3])
  Qbuildataweek<-as.numeric(arimaterms[4])
  qbuildataday<-as.numeric(arimaterms[7])
  Qbuildataday<-as.numeric(arimaterms[8])
  
  dbuildataday<-as.numeric(arimaterms[11])
  Dbuildataday<-as.numeric(arimaterms[12])
  
  dbuildataweek<-as.numeric(arimaterms[9])
  Dbuildataweek<-as.numeric(arimaterms[10])
  
  buildfitday<-Arima(BXCXbuildatadaytrain,
                     order=c(pbuildataday,dbuildataday,qbuildataday),
                     seasonal=c(Pbuildataday,Dbuildataday,Qbuildataday),
                     lambda=BXCXLAMbuildataday,include.drift=F,
                     include.constant=F, include.mean=F,method="CSS")
  resumen<-summary(buildfitday)
  confint(buildfitday)
  plot.ts(buildfitday$residuals)
  autoplot(buildfitday)
  coeftest(buildfitday)
  preds<-forecast(buildfitday,hoursforecast)
  accuracy(preds,BXCXbuildataday[(daystrain+1):(daystrain+24)])
  plot(preds,xlim=c(270,330))
  return(resumen)
}

arimaans<-all_arima(filename = "B002",location = locations)
sarimatermsB116=c(8,3,7,4,8,5,1,3,arimaans[7],arimaans[8],arimaans[5],arimaans[6])
sarimatermsB002=c(6,3,8,4,6,13,4,5,arimaans[7],arimaans[8],arimaans[5],arimaans[6])

arimafit_ans<-all_arima_fit(filename="B002",location=locations,
                            arimaterms=sarimatermsB002,hoursforecast=(24*7*3),
                            daystrain=(24*40*7+(24*0)))


all_arima_continue<-function(filename, location="",ariterms){
  setwd(paste("D:/ThesisExperiments/SelectedBuildings/",filename, sep = ""))
  #filename = "B129"
  #location="D:/ThesisExperiments/Original data/Energy/"
  buildata<-read.csv(paste(location, filename, ".csv", sep = ""))
  buildatatsday<-ts(buildata$Ef_kWh,frequency = 24)
  buildatatsweek<-ts(buildata$Ef_kWh,frequency = (24*7))
  buildatatsdaytrain<-ts(buildata$Ef_kWh[1:6132],frequency = 24)
  buildatatsweektrain<-ts(buildata$Ef_kWh[1:6132],frequency = (24*7))
  # 
  # #dev.off()
  # jpeg(filename = paste("Day alldata seasonal decom ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # plot(stl(buildatatsday, "per"),main=paste(filename, "by days, Seasonal Decomposition"))
  # dev.off()
  # jpeg(filename = paste("Week alldata seasonal decom ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # plot(stl(buildatatsweek, "per"),main=paste(filename, " by weeks, Seasonal Decomposition"))
  # dev.off()
  # jpeg(filename = paste("Day train seasonal decom  ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # plot(stl(buildatatsdaytrain, "per"),main=paste(filename, "by days, Seasonal Decomposition"))
  # dev.off()
  # jpeg(filename = paste("Week train seasonal decom  ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # plot(stl(buildatatsweektrain, "per"),main=paste(filename, " by weeks, Seasonal Decomposition"))
  # dev.off()
  # jpeg(filename = paste("Day alldata seasonal ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # seasonplot(buildatatsday, season.labels = T, main=paste("Seasonal Plot",filename,"by days"))
  # dev.off()
  # jpeg(filename = paste("Week alldata seasonal ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # seasonplot(buildatatsweek, season.labels = T,main=paste("Seasonal Plot",filename,"by weeks"))
  # dev.off()
  # jpeg(filename = paste("Day train seasonal ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # seasonplot(buildatatsdaytrain, season.labels = T, main=paste("Seasonal Plot",filename,"by days"))
  # dev.off()
  # jpeg(filename = paste("Week train seasonal ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # seasonplot(buildatatsweektrain, season.labels = T,main=paste("Seasonal Plot",filename,"by weeks"))
  # dev.off()
  # 
  # jpeg(filename = paste("Week alldata especgram ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # buildataesweek<-spec.pgram(buildatatsweek,log="no")
  # dev.off()
  # jpeg(filename = paste("Day alldata especgram ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # buildataesday<-spec.pgram(buildatatsday,log="no")
  # dev.off()
  # jpeg(filename = paste("Week train especgram ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # buildataesweektrain<-spec.pgram(buildatatsweektrain,log="no")
  # dev.off()
  # jpeg(filename = paste("Day train especgram ",filename,".jpeg",sep = "") , height = 786, width = 1048)
  # buildataesdaytrain<-spec.pgram(buildatatsdaytrain,log="no")
  # dev.off()
  # 
  orderedweekbuildata<-order(buildataesweek$spec,buildataesweek$freq, decreasing = T)#ordena los datos del espectrograma de mayor a menor
  ...
  if(dbuildataday!=0){
    ...
    autoplot(diff(BXCXbuildatadaytrain,lag = (frequency(buildatatsdaytrain)*Dbuildataday)), main=paste(filename, " stationary and non seasonal time series by day", sep=" ")) #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
  }

  dev.off()
  
  jpeg(filename = paste("Autocorrelation plot ",filename," day time series.jpeg",sep = "") , height = 786, width = 1048)
  autocorplot<-ggAcf(residuals(Arima(BXCXbuildatadaytrain,order=c(0,dbuildataday,0),seasonal = c(0,Dbuildataday,0),lambda = BXCXLAMbuildataday,include.drift = F,include.constant = T, include.mean = T)),lag.max = (50)) #grafico de autocorrelacion, contar las lineas consecutivas que se salen de la franja azul
  print(autocorplot)
  dev.off()
  
  jpeg(filename = paste("Parcial Autocorrelation plot ",filename," day time series.jpeg",sep = "") , height = 786, width = 1048)
  parautocorplot<-ggPacf(residuals(Arima(BXCXbuildatadaytrain,order = c(0,0,0), seasonal = c(0,0,0),lambda = BXCXLAMbuildataday)),lag.max = 50)#grafico de autocorrelacion parcial, contar las lineas que se salen de la franja azul
  print(parautocorplot)
  dev.off()
  ...
  buildatafitday<-Arima(BXCXbuildatadaytrain, order = c(pbuildataday,dbuildataday,qbuildataday), seasonal=c(Pbuildataday,Dbuildataday,Qbuildataday),lambda = BXCXLAMbuildataday, include.drift = F,include.constant = F, include.mean = F, method = "CSS")
  jpeg(filename = paste("ARIMA 1week forecast for ",filename," day time series.jpeg",sep = "") , height = 786, width = 1048)
  plot(forecast(buildatafitday, (24*7*4)))
  dev.off()
  
  jpeg(filename = paste("Zoom ARIMA 1week forecast for ",filename," day time series.jpeg",sep = "") , height = 786, width = 1048)
  plot(forecast(buildatafitday, (24*7*4)),xlim=c(230,290))
  dev.off()
  
  buildataforeday<-forecast(buildatafitday, (24*7*4))
  jpeg(filename =paste("ARIMA forecast comparison for ", filename,".jpeg"), height = 786, width = 1048)
  plot(1:(24*7*4),InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday),ylim=c(min(InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday),buildatatsday[6133:(6132+24*7*4)]),max(InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday),buildatatsday[6133:(6132+24*7*4)])+20),main= paste("Forecast vs. real data for ", filename),ylab="KWh", xlab="hours")
  par(new=T)
  plot(1:(24*7*4),buildatatsday[6133:(6132+24*7*4)],col="red",ylim=c(min(InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday),buildatatsday[6133:(6132+24*7*4)]),max(InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday),buildatatsday[6133:(6132+24*7*4)])+20),ylab="KWh",xlab="hours")
  legend((24*7*3.5),max(buildatatsday[6133:(6132+24*7*4)])+20,legend=c("Forecast","Real data"),lwd=c(2,1),col=c("black","red"))
  dev.off()
  buildataerror<-accuracy(InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday),buildatatsday[6133:(6132+24*7*4)])
  MAPE<- sum(abs(InvBoxCox(buildataforeday$mean, lambda = BXCXLAMbuildataday)-buildatatsday[6133:(6132+24*7*4)])/buildatatsday[6133:(6132+24*7*4)])/length(buildatatsday[6133:(6132+24*7*4)])
  ordereddata<-list(orderedweekbuildata,ordereddaybuildata,aaday,aaweek,dbuildataday,Dbuildataday,buildataforeday,buildataerror, MAPE)
  return(ordereddata)
}


arimaB125<-all_arima_continue(filename = "B125",ariterms = c(12,1,3,4,1,2))
arimab126<-all_arima_continue(filename = "B126",ariterms = c(8,1,2,9,1,8))
arimab129<-all_arima_continue(filename = "B129",ariterms = c(7,1,4,6,1,3))
arimab132<-all_arima_continue(filename = "B132",ariterms = c(2,1,1,4,1,2))
arimab133<-all_arima_continue(filename = "B133",ariterms = c(23,0,22,3,1,0))
arimab199<-all_arima_continue(filename = "B199",ariterms = c(22,1,0,5,1,1))
arimab200<-all_arima_continue(filename = "B200",ariterms = c(10,0,22,7,1,0))
arimab201<-all_arima_continue(filename = "B201",ariterms = c(2,1,5,3,1,3))
arimab202<-all_arima_continue(filename = "B202",ariterms = c(2,1,5,3,1,3))


############# CHAP 0 PART 1 READ THE DATA #########################

##usar edificios 129 a 133 


#setwd("/home/jose/Downloads/data colab U central") #cambia ubicacion
#setwd("C:/Users/JOSE/Downloads/data colab U central/data colab U central") #cambia ubicacion
...
B129<-read.csv("D:/ThesisExperiments/Original data/Energy/B129.csv")
B130<-read.csv("D:/ThesisExperiments/Original data/Energy/B130.csv")
B131<-read.csv("D:/ThesisExperiments/Original data/Energy/B131.csv")
B132<-read.csv("D:/ThesisExperiments/Original data/Energy/B132.csv")
B133<-read.csv("D:/ThesisExperiments/Original data/Energy/B133.csv")

############ CHAP 0 PART 2 BOXPLOT FOR EACH BUILDING ##################
# jpeg(filename = "B116_Boxplot.jpeg", height = 786, width = 1048)
# boxplot(B116$Ef_kWh, main="Box Plot B116") #diagrama de caja que evidencia la presencia de datos an????malos
# dev.off()
...
B129.tsweek<-ts(B129$Ef_kWh, frequency = (24*7))
B129.tsday<-ts(B129$Ef_kWh, frequency = (24))

B130.tsweek<-ts(B130$Ef_kWh, frequency = (24*7))
B130.tsday<-ts(B130$Ef_kWh, frequency = (24))

B131.tsweek<-ts(B131$Ef_kWh, frequency = (24*7))
B131.tsday<-ts(B131$Ef_kWh, frequency = (24))

B132.tsweek<-ts(B132$Ef_kWh, frequency = (24*7))
B132.tsday<-ts(B132$Ef_kWh, frequency = (24))

B133.tsweek<-ts(B133$Ef_kWh, frequency = (24*7))
B133.tsday<-ts(B133$Ef_kWh, frequency = (24))
...
# 
# plot(stl(B002.tsday, "per"),main="B002 by days, Seasonal Decomposition (half of data)") #grafica de descomposicion de serie de timepo
...
plot(stl(B129.tsday, "per"),main="B129 by days, Seasonal Decomposition")
plot(stl(B129.tsweek, "per"),main="B129 by weeks, Seasonal Decomposition")

plot(stl(B130.tsday, "per"),main="B130 by days, Seasonal Decomposition")
plot(stl(B130.tsweek, "per"),main="B130 by weeks, Seasonal Decomposition")

plot(stl(B131.tsday, "per"),main="B131 by days, Seasonal Decomposition")
plot(stl(B131.tsweek, "per"),main="B131 by weeks, Seasonal Decomposition")

plot(stl(B132.tsday, "per"),main="B132 by days, Seasonal Decomposition")
plot(stl(B132.tsweek, "per"),main="B132 by weeks, Seasonal Decomposition")

plot(stl(B133.tsday, "per"),main="B133 by days, Seasonal Decomposition")
plot(stl(B133.tsweek, "per"),main="B133 by weeks, Seasonal Decomposition")

############ CHAP 1 PART 3 SEASON PLOT FOR EACH TIME SERIES#################
#grafica los datos segun la frecuencia que se haya incluido

seasonplot(B129.tsday, season.labels = T, main="Seasonal Plot B129 by days")
seasonplot(B129.tsweek, season.labels = T,main="Seasonal Plot B129 by weeks")

seasonplot(B130.tsday, season.labels = T, main="Seasonal Plot B130 by days")
seasonplot(B130.tsweek, season.labels = T,main="Seasonal Plot B130 by weeks")

seasonplot(B131.tsday, season.labels = T, main="Seasonal Plot B131 by days")
seasonplot(B131.tsweek, season.labels = T,main="Seasonal Plot B131 by weeks")

seasonplot(B132.tsday, season.labels = T, main="Seasonal Plot B132 by days")
seasonplot(B132.tsweek, season.labels = T,main="Seasonal Plot B132 by weeks")

seasonplot(B133.tsday, season.labels = T, main="Seasonal Plot B133 by days")
seasonplot(B133.tsweek, season.labels = T,main="Seasonal Plot B133 by weeks")
...
# BXCXB116week<-BoxCox(B116.tsweek,BXCXLAMB116day) #hace la transformacion boxcox
# 
...
# B116saeasadjdia<-seasadj(stl(BXCXB116dia,s.window = "periodic")) #muestra la gr????fica de ajuste a la periodicidad
# plot(B116saeasadjdia)
# B002saeasadjdia<-seasadj(stl(BXCXB002dia,s.window = "periodic")) #muestra la gr????fica de ajuste a la periodicidad
...
autoplot(diff(diff(BXCXB116day, lag = 1),lag = 24), main="B116 stationary and non seasonal time series by day") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24

autoplot(diff(diff(BXCXB002day, lag = 1),lag = 24), main="B002 stationary and non seasonal time series by day") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24

autoplot(diff(diff(BXCXB011day, lag = 1),lag = 24), main="B011 stationary and non seasonal time series by day") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24

autoplot(diff(diff(BXCXB123day, lag = 1),lag = 24), main="B123 stationary and non seasonal time series by day") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24

autoplot(diff(diff(BXCXB124day, lag = 1),lag = 24), main="B124 stationary and non seasonal time series by day") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24


# autoplot(diff(diff(BXCXB116week, lag = 1),lag = 24), main="B116 stationary and non seasonal time series by week") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
# 
# autoplot(diff(diff(BXCXB002week, lag = 1),lag = 24), main="B002 stationary and non seasonal time series by week") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
# 
# autoplot(diff(diff(BXCXB011week, lag = 1),lag = 24), main="B011 stationary and non seasonal time series by week") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
# 
# autoplot(diff(diff(BXCXB123week, lag = 1),lag = 24), main="B123 stationary and non seasonal time series by week") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
# 
# autoplot(diff(diff(BXCXB124week, lag = 1),lag = 24), main="B124 stationary and non seasonal time series by week") #transforma la serie en estacionaria con el lag 1 y en estacional con el 24
...
ggAcf(residuals(Arima(BXCXB116day,order=c(0,1,0),seasonal = c(0,1,0),lambda = BXCXLAMB116day,include.drift = F,include.constant = T, include.mean = T)),lag.max = (50)) #grafico de autocorrelacion, contar las licis que se salen de la franja azul
### resultado q=2, que son los primeros de la grafica y Q=2 que se cuenta desde el 24 incluido
qB116day<-1
QB116day<-2
...
ggAcf(residuals(Arima(BXCXB002day,order=c(0,1,0),seasonal = c(0,1,0),lambda = BXCXLAMB002day,include.drift = F,include.constant = T, include.mean = T)),lag.max = (50)) #grafico de autocorrelacion, contar las lineas que se salen de la franja azul
...
ggPacf(residuals(Arima(BXCXB116day,order = c(0,0,0), seasonal = c(0,0,0),lambda = BXCXLAMB116day)),lag.max = 50)#grafico de autocorrelacion parcial, contar las lineas que se salen de la franja azul
### resultado p=2, que son los que se toman desde el comienzo de la gr????fica y P=3 que se toman desde el 24 incluido
pB116day<-2
PB116day<-4
...
##by weeks
# 
# ggAcf(residuals(Arima(BXCXB116week,order=c(0,1,0),seasonal = c(0,1,0),lambda = BXCXLAMB116week,include.drift = F,include.constant = T, include.mean = T)),lag.max = (180)) 
# ### resultado q=2, que son los primeros de la gr????fica y Q=2 que se cuenta desde el 24 incluido
# qB116week<-7
# QB116week<-1
# 
...
# 
# 
# ggPacf(residuals(Arima(BXCXB116week,order = c(0,1,0), seasonal = c(0,1,0),lambda = BXCXLAMB116week)),lag.max = 180)
# ### resultado p=2, que son los que se toman desde el comienzo de la gr????fica y P=3 que se toman desde el 24 incluido
# pB116week<-8
# PB116week<-3
# 
...
#ggAcf(residuals(Arima(BXCXB116),lambda=BXCXLAMB116),lag.max = 50) #grafico de autocorrelacion con lambda calculado del boxcox
#ggAcf(residuals(auto.arima(BXCXB116,lambda=BXCXLAMB116)),lag.max = 50) #grafico de autocorrelacion con el autoarima
#ggPacf(residuals(Arima(BXCXB116,order = c(0,0,0), seasonal = c(0,0,0),lambda = BXCXLAMB116)),lag.max = 50) #grafico de autocorrelacion parcial

##sin componente estacional con componente estacionario
#ggAcf(residuals(Arima(BXCXB116,order=c(0,1,0),seasonal = c(0,0,0),lambda = BXCXLAMB116,include.drift = T,include.constant = T, include.mean = T)),lag.max = 50) #grafico de autocorrelacion
#ggAcf(residuals(auto.arima(BXCXB116,lambda=BXCXLAMB116)),lag.max = 50) #grafico de autocorrelaci????n con el autoarima




##con componentes estacional y estacionarios
#ggAcf(residuals(Arima(BXCXB116,order=c(0,1,0),seasonal = c(0,1,0),lambda = BXCXLAMB116,include.drift = T,include.constant = T, include.mean = T)),lag.max = 50) #grafico de autocorrelacion
#ggAcf(residuals(auto.arima(BXCXB116,lambda=BXCXLAMB116)),lag.max = 50) #grafico de autocorrelacion con el autoarima
...
#ggAcf(residuals(Arima(BXCXB116,order=c(0,0,0),seasonal = c(0,1,0),lambda = BXCXLAMB116,include.drift = T,include.constant = T, include.mean = T)),lag.max = 50) #grafico de autocorrelacion
#ggAcf(residuals(auto.arima(BXCXB116,lambda=BXCXLAMB116)),lag.max = 50) #grafico de autocorrelacion con el autoarima
...
##aqu??? el auto arima me dice cuales son los valores de p,d y q pero no se cuales quedan para P, D y Q
...
B116fitday<-Arima(BXCXB116day, order = c(pB116day,dB116day,qB116day), seasonal=c(PB116day,DB116day,QB116day),lambda = BXCXLAMB116day, include.drift = F,include.constant = F, include.mean = F, method = "CSS")
summary(B116fitday)
confint(B116fitday)
...
# B116fitweek<-Arima(BXCXB116week, order = c(pB116week,dB116week,qB116week), seasonal=c(PB116week,DB116week,QB116week),lambda = BXCXLAMB116week, include.drift = F,include.constant = F, include.mean = F, method = "CSS")
# summary(B116fitweek)
...
plot(1:672,InvBoxCox(B116.foreday$mean, lambda = BXCXLAMB116day),ylim=c(min(B116.ts2day[4393:5064]),max(B116.ts2day[4393:5064])+20),main="B116 forecast vs. real data",ylab="KWh")
par(new=T)
plot(1:672,InvBoxCox(BXCXB116day2,lambda = BXCXLAMB116day)[4393:(4392+24*7*4)],col="red",ylim=c(min(B116.ts2day[4393:5064]),max(B116.ts2day[4393:5064])+20),ylab="KWh")
...
plot(1:672,InvBoxCox(B123.foreday$mean, lambda = BXCXLAMB123day),ylim=c(min(B123.ts2day[4393:5064]),max(B123.ts2day[4393:5064])+20),main="B123 forecast vs. real data",ylab="KWh")
par(new=T)
plot(1:672,InvBoxCox(BXCXB123day2,lambda = BXCXLAMB123day)[4393:(4392+24*7*4)],col="red",ylim=c(min(B123.ts2day[4393:5064]),max(B123.ts2day[4393:5064])+20),ylab="KWh")
...
plot(1:672,InvBoxCox(B124.foreday$mean, lambda = BXCXLAMB124day),ylim=c(min(B124.ts2day[4393:5064]),max(B124.ts2day[4393:5064])+5),main="B124 forecast vs. real data",ylab="KWh")
par(new=T)
plot(1:672,InvBoxCox(BXCXB124day2,lambda = BXCXLAMB124day)[4393:(4392+24*7*4)],col="red",ylim=c(min(B124.ts2day[4393:5064]),max(B124.ts2day[4393:5064])+5),ylab="KWh")
...
plot(1:672,InvBoxCox(B002.foreday$mean, lambda = BXCXLAMB002day),ylim=c(min(B002.ts2day[4393:5064])-10,max(B002.ts2day[4393:5064])+50),main="B002 forecast vs. real data",ylab="KWh")
par(new=T)
plot(1:672,InvBoxCox(BXCXB002day2,lambda = BXCXLAMB002day)[4393:(4392+24*7*4)],col="red",ylim=c(min(B002.ts2day[4393:5064])-10,max(B002.ts2day[4393:5064])+50),ylab="KWh")
...
plot(1:672,InvBoxCox(B011.foreday$mean, lambda = BXCXLAMB011day),ylim=c(min(B011.ts2day[4393:5064]),max(B011.ts2day[4393:5064])+2),main="B011 forecast vs. real data",ylab="KWh")
par(new=T)
plot(1:672,InvBoxCox(BXCXB011day2,lambda = BXCXLAMB011day)[4393:(4392+24*7*4)],col="red",ylim=c(min(B011.ts2day[4393:5064]),max(B011.ts2day[4393:5064])+2),ylab="KWh")
...
###########haciendo los terminos de fourier separando d???as entre semana y fines de semana
...
autoplot(diff(diff(BXCXB116wkday, lag = 1),lag = 24), main="B116 weekdays stationary and non seasonal time series")
autoplot(diff(diff(BXCXB116wkend, lag = 1),lag = 24), main="B116 weekend stationary and non seasonal time series")

autoplot(diff(diff(BXCXB002wkday, lag = 1),lag = 24), main="B002 weekdays stationary and non seasonal time series")
autoplot(diff(diff(BXCXB002wkend, lag = 1),lag = 24), main="B002 weekend stationary and non seasonal time series")
...
plot(1:240,InvBoxCox(foreB116wkday$mean,lambda = BXCXLAMB116wkday),xlim=c(-5,250), ylim=c(25,100),col="red",xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B116ts.wkdays2[3121:(3120+240)],xlim=c(-5,250), ylim=c(25,100), col="blue",xlab="Time", ylab="Kwh",main="B116 weekdays: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB002wkday$mean,lambda = BXCXLAMB002wkday),xlim=c(-5,250), ylim=c(35,150),col="red",xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B002ts.wkdays2[3121:(3120+240)],col="blue",xlim=c(-5,250), ylim=c(35,150),xlab="Time", ylab="Kwh",main="B002 weekdays: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB011wkday$mean,lambda = BXCXLAMB011wkday),xlim=c(-5,250), ylim=c(5,12),col="red",xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B011ts.wkdays2[3121:(3120+240)],col="blue",xlim=c(-5,250), ylim=c(5,12),xlab="Time", ylab="Kwh",main="B011 weekdays: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB123wkday$mean,lambda = BXCXLAMB123wkday),xlim=c(-5,250), ylim=c(55,140),col="red",xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B123ts.wkdays2[3121:(3120+240)],xlab="Time", ylab="Kwh",main="B123 weekdays: forecast vs. real data",xlim=c(-5,250), ylim=c(55,140),col="blue")
...
plot(1:240,InvBoxCox(foreB124wkday$mean,lambda = BXCXLAMB124wkday),xlim=c(-5,250), ylim=c(5,33),col="red",xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B124ts.wkdays2[3121:(3120+240)],xlim=c(-5,250), ylim=c(5,33),col="blue",xlab="Time", ylab="Kwh",main="B124 weekdays: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB116wkend$mean,lambda = BXCXLAMB116wkend),col="red",ylim=c(min(B116ts.wkend2[1249:(1248+240)]),max(B116ts.wkend2[1249:(1248+240)])),xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B116ts.wkend2[1249:(1248+240)],col="blue",ylim=c(min(B116ts.wkend2[1249:(1248+240)]),max(B116ts.wkend2[1249:(1248+240)])),xlab="Time", ylab="Kwh",main="B116 weekends: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB002wkend$mean,lambda = BXCXLAMB002wkend),col="red",ylim=c(min(B002ts.wkend2[625:(624+240)])-5,max(B002ts.wkend2[625:(624+240)]+5)),xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B002ts.wkend2[625:(624+240)],col="blue",ylim=c(min(B002ts.wkend2[625:(624+240)])-5,max(B002ts.wkend2[625:(624+240)]+5)),xlab="Time", ylab="Kwh",main="B002 weekends: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB011wkend$mean,lambda = BXCXLAMB011wkend),col="red",ylim=c(min(B011ts.wkend2[625:(624+240)]),max(B011ts.wkend2[625:(624+240)])),xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B011ts.wkend2[1249:(1248+240)],col="blue",ylim=c(min(B011ts.wkend2[625:(624+240)]),max(B011ts.wkend2[625:(624+240)])),xlab="Time", ylab="Kwh",main="B011 weekends: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB123wkend$mean,lambda = BXCXLAMB123wkend),col="red",ylim=c(min(B123ts.wkend2[1249:(1248+240)]),max(B123ts.wkend2[1249:(1248+240)])+5),xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B123ts.wkend2[1249:(1248+240)],col="blue",ylim=c(min(B123ts.wkend2[1249:(1248+240)]),max(B123ts.wkend2[1249:(1248+240)])+5),xlab="Time", ylab="Kwh",main="B123 weekends: forecast vs. real data")
...
plot(1:240,InvBoxCox(foreB124wkend$mean,lambda = BXCXLAMB124wkend),col="red",ylim=c(min(B124ts.wkend2[1249:(1248+240)]),max(B124ts.wkend2[1249:(1248+240)])),xlab="Time", ylab="Kwh")
par(new=T)
plot(1:240,B124ts.wkend2[1249:(1248+240)],col="blue",ylim=c(min(B124ts.wkend2[1249:(1248+240)]),max(B124ts.wkend2[1249:(1248+240)])),xlab="Time", ylab="Kwh",main="B124 weekends: forecast vs. real data")
